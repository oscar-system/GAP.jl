...
if install_gap
    ...
    if !isfile(gap_source_tarball) || !check_hash(gap_source_tarball, SHA.sha512, sources_sha512[gap_source_tarball])
        println("Sourced for GAP $gap_version don't exist, or don't match the hash, downloading...")
        ...
        # download("https://www.gap-system.org/pub/gap/gap-4.$(gap_version.minor)/tar.bz2/$(gap_source_tarball)")
        download("https://github.com/gap-system/gap/releases/download/v$(gap_version)/$(gap_source_tarball)")
    end
