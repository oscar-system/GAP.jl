...
@testset "interrupt bridge" begin
    ...
    request_interrupt() = @ccall GAP.JuliaInterface_path.JuliaInterface_RequestGapInterrupt()::Cvoid

    request_interrupt()
    @test_throws InterruptException GAP.Globals.gapjl_signals_spin()

    # GAP acts on an interrupt at its next statement; a call that executes
    # none must not leave it armed for a later, unrelated call
    request_interrupt()
    @test_throws InterruptException GAP.Globals.LEN_LIST(GapObj([1, 2, 3]))
