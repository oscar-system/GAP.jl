...
const _gaproot_scratchkey = Ref{String}()
const _gaproot = Ref{String}()

function gaproot()
    if !isassigned(_gaproot_scratchkey)
        str1 = Dates.format(Dates.now(), Dates.dateformat"yyyy-mm-dd\THH-MM-SS.s") # ISO 8601 but with - instead of :
        str2 = Random.randstring(10)
        _gaproot_scratchkey[] = "gap_" * str1 * "_" * str2
    end
    if !isassigned(_gaproot)
        _gaproot[] = @get_scratch!(_gaproot_scratchkey[])
    end
    return _gaproot[]
end
