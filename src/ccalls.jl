...
#
# The first argument is a return type hint: each `return` carries a type
# assertion, so the inferred return type is (a subtype of) `T`. This matters
# because an assertion at the call site (as in code generated by `@wrap`)
# would come too late to prevent boxing: without the hint the inferred return
# type is `Any`, hence even immediate integers must be boxed. For small unions
# like `GapInt` the union-splitting machinery instead passes them along
# unboxed.
#
# For values not matching `T` this throws a `TypeError`, just as an assertion
...
# with `T` equal to `Any` all the type assertions above are no-ops
_GAP_TO_JULIA(ptr::Ptr{Cvoid}) = _GAP_TO_JULIA(Any, ptr)

else

# On older Julia versions the method above cannot be inlined into its callers
# (see `@wrap` in `macros.jl`), so delegating to it would cost an additional
# allocation per call; hence a copy of its body without the assertions.
...
# ... and for the same reason the untyped conversion is used for `Any`
