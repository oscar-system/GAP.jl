...
# Variant of `_GAP_TO_JULIA` taking a return type hint `T`.
#
# The plain `_GAP_TO_JULIA` has inferred return type `Any`, hence values it
# returns must be boxed, even immediate integers. A type assertion at the call
# site (as in code generated by `@wrap`) comes too late to prevent that.
# Here each `return` carries the assertion, so the inferred return type is
# (a subtype of) `T`; for small unions like `GapInt` the union-splitting
# machinery then passes immediate integers along unboxed.
#
# For values not matching `T` this throws a `TypeError`, just as the call
# site assertion did before.
#
# With `T` equal to `Any` all these assertions are no-ops, so one is tempted
# to drop the method above and define it as `_GAP_TO_JULIA(Any, ptr)`. That
# is indeed equivalent on Julia >= 1.12, but on older versions it costs an
# extra allocation per call: as described for `@wrap` in `macros.jl`, this
# method cannot be inlined into its callers there, so the delegation results
# in a second outlined call.
Base.@assume_effects :foldable !:consistent @inline function _GAP_TO_JULIA(::Type{T}, ptr::Ptr{Cvoid}) where {T}
    ptr == C_NULL && return nothing::T
    # convert immediate ints and FFEs directly, to avoid (un)boxing
    as_int = reinterpret(Int, ptr)
    as_int & 1 == 1 && return (as_int >> 2)::T
    as_int & 2 == 2 && return reinterpret(FFE, ptr)::T
    tnum = TNUM_OBJ(ptr)
    if tnum < FIRST_EXTERNAL_TNUM
        if tnum == T_BOOL
            ptr == _GAP_True[] && return true::T
            ptr == _GAP_False[] && return false::T
        end
        return unsafe_pointer_to_objref(ptr)::T
    end
    return (@ccall JuliaInterface_path.julia_gap(ptr::Ptr{Cvoid})::Any)::T
end

_GAP_TO_JULIA(::Type{Any}, ptr::Ptr{Cvoid}) = _GAP_TO_JULIA(ptr)
...
call_gap_func_nokw(func::GapObj, args...) = call_gap_func_nokw(Any, func, args...)

# variant converting the result with a return type hint `T`, so that
# e.g. immediate integer results need not be boxed
# (see `_GAP_TO_JULIA(T, ptr)`); used by code generated by `@wrap`
@inline function call_gap_func_nokw(::Type{T}, func::GapObj, args...) where {T}
    if is_func(func) && length(args) <= 6
        ret = _call_gap_func(func, args...)
    else
        ret = slow_call_gap_func_nokw(func, args)
    end
    return _GAP_TO_JULIA(T, ret)
end
